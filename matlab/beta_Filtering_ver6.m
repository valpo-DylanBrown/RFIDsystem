...
readAPIKey = 'IIIFB0QBW67JCGTU';

%Input the Channel ID for your ThingSpeak Server
writeChannelID = channelID;
% Enter your Write API key (' ')
writeAPIKey = '4RS2DMH6YGBDXDHL';
%% Read Data %%
...
% FIXED HERE
hoursUntilNotPresent = 12;
t1 = datetime('now')-hours(hoursUntilNotPresent);
t2 = datetime('now');
S=timerange(t1, t2);
...
% Add a column called "Date" and put today's date in
T.Date = repmat({datestr(now, 02)}, height(T), 1); 
% add a column called "time" and put the time a student checked in.
%T.Time = zeros(height(T),1);
...
writetable(Tout, 'out.xlsx', 'FileType','spreadsheet','Sheet','Attendance');
...
% Others Involved: Daniel Piccoli and Lindsey Tryban
% Advisor - Dan White